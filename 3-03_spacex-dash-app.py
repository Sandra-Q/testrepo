...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        # Group by Launch Site and calculate total success counts
        site_success = spacex_df.groupby('Launch Site')['class'].sum().reset_index()
        site_success.columns = ['Launch Site', 'Total Success']
        
        # Create pie chart showing success counts for all sites
        fig = px.pie(
            site_success,
        ...
        # Count success (class=1) and failure (class=0) launches
        ...
        # Create pie chart showing success vs failure for selected site
        fig = px.pie(
            success_counts,
            values='Count',
            names='Outcome',
...
def get_scatter_chart(entered_site, payload_range):
    ...
    # Select title and data based on site selection
    if entered_site == 'ALL':
        # Create scatter plot for all sites with payload range filter
        fig = px.scatter(
            filtered_df,
            x='Payload Mass (kg)',
            y='class',
